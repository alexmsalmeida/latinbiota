...
metadata = metadata[which(metadata$`Health state` == "Healthy" & metadata$`Age group` == "Adult" & metadata$Antibiotics != "Yes"),]
...
wrld.plot = ggplot() + 
  ...
  coord_map() + 
  labs(x="", y="") + 
  theme(panel.background = element_rect(fill = "white"), panel.border = element_rect(colour = "black", size=0.2, fill=NA), 
...
cont.prop = ggplot(ddf.cont, aes(x=continent, y=proportion, fill=continent)) +
  ...
  scale_fill_manual(values=rev(brewer.pal(6, "Set3"))) +
  guides(fill=FALSE) +
...
ggarrange(wrld.plot, cont.prop, nrow=1, widths=c(2,1), labels=c("", ""), font.label = list(size = 18, color = "black", face = "bold", family = NULL))
#ggsave(file="map_barplot_all.png", height=5, width=13, dpi=300)
ggsave(file="map_barplot_healthy-adults.png", height=5, width=13, dpi=300)
