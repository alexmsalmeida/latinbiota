...
file = "CAZy_annotations.tsv"
no_col = max(count.fields(file, sep = "\t"))
cazy.data = read.table(file, sep="\t", fill=TRUE, header = F, col.names=c(1:no_col))
cazy.descr = read.delim("cazy_descriptions.tsv", header = FALSE, stringsAsFactors = FALSE)
...
cazy.plot = ggplot(cazy.sign, aes(x=CAZy, y=Novel_present, fill=result)) +
  ...
  scale_x_discrete(limits=cazy.sign[order(cazy.sign$Novel_present, decreasing=TRUE)[1:25],"CAZy"],
                   labels=cazy.sign[order(cazy.sign$Novel_present, decreasing=TRUE)[1:25],"funcs"]) +
...
ggsave("cazy_enriched.png", dpi=300, height=5, width=6)