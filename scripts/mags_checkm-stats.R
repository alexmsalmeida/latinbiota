...
checkm.stats = read.csv("all_checkm.csv")
checkm.stats$genome = gsub(".fa", "", checkm.stats$genome)
...
checkm.stats$MIMAG = factor(checkm.stats$MIMAG, levels=c("Near complete", "Medium quality"))

# plots
scatter.plot = ggplot(checkm.stats, aes(x=completeness, y=contamination, colour=MIMAG)) + 
  geom_point(size=0.8, alpha=0.5) + 
  scale_color_manual(values=c("steelblue", "darkolivegreen3"), name="") + 
...
comp.dens = ggplot(checkm.stats, aes(x=completeness, fill=MIMAG)) + 
  ...
  scale_fill_manual(values=c("steelblue", "darkolivegreen3")) + 
  theme_bw() + 
  guides(fill=FALSE) +
  ylab("\n") + 
...
ggsave("mag_quality.tiff", height=6, width=8, dpi=300)