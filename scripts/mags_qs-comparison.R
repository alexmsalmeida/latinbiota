...
uhgg.freq = data.frame(table(dnadiff.uhgg$UHGG_species))
write.table(uhgg.freq, file="uhgg_frequency.tsv", col.names=FALSE, row.names=FALSE, quote=FALSE, sep="\t")
...
ggarrange(qs.plot, bar.qs.plot, ncol=2, nrow=1, widths=c(2.5,1), common.legend = TRUE)
ggsave(file="qs_comparison.pdf", height=6, width=8)

# save data
isolates = genome2species[which(genome2species$Genome_type == "Isolate"), "Genome"]
replace.mags = qs.df[which(qs.df$QS_result == "Latinbiota better" & !qs.df$UHGG_species %in% isolates),c("qry", "UHGG_species")]
write.table(replace.mags, file="mag_replacement.tsv", sep="\t", row.names=FALSE, quote=FALSE, col.names=FALSE)