...
bwa.data = as.data.frame(read.csv("viruses/mapping/summary_tables/viruses-above0_binary_world-adult-healthy.csv", row.names=1, check.names=FALSE, stringsAsFactors = FALSE)); variable = "Continent"
...
# group by metadata
bwa.data$Variable = metadata[rownames(bwa.data), "Continent"]
...
upset.df = t(upset.df)
upset.df[upset.df/meta.counts$Freq < 0.10] = 0
...
# add source
gen.stats = read.delim("viruses/species_gpd-latinbiota.tsv", stringsAsFactors = FALSE)
upset.df$Source = gen.stats[match(rownames(upset.df), gen.stats$Code),"Source"]

# upset plot
upset(upset.df, colnames(upset.df)[-((ncol(upset.df) - 1):ncol(upset.df))], n_intersections=10, name="",
      base_annotations=list('Intersection size'=intersection_size(
        mapping=aes(fill=Source)) 
        + scale_fill_manual(values=c('Latinbiota'='brown3', 'Shared'='palegreen4', 'GPD'='steelblue'), name = "Genome source")),
      annotations = list(
        taxon=(
...
ggsave(file="../../../Publications/2022-Latinbiota/Figures/extfig7/upset_viruses_world-adult-healthy.pdf", height=9, width=10)
