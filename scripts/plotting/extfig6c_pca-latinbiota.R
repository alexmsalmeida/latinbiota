...
bwa.prev = as.data.frame(read.csv("prokaryotes/mapping/summary_tables/euproks-above0_binary_latinbiota-all.csv", row.names=1, check.names=FALSE))
...
pca.plot = ggplot(bwa.df.pca, aes(x=PC1, y=PC2, colour=Country)) + 
  geom_point(size=0.8, alpha=1) +
  theme_classic() + 
  theme(panel.grid.minor = element_blank()) + 
  theme(legend.pos = "top") +
  scale_colour_manual(values=c("#FFD92F", "#A6D854", "#E78AC3", "#71d1ff", "#FC8D62", "#b2450a", "#3b35ce", "darkgrey")) +
...
ggsave(file="../../../Publications/2022-Latinbiota/Figures/extfig6/pca_latinbiota.pdf", width=7, height=5)

# calculate % explained variance by factor
perm.res = adonis2(bwa.prev ~ Country, metadata, method="jaccard")
