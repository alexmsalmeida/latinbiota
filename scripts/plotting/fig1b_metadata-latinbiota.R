...
age$Type = "Age group"
lfst = reshape2::melt(table(metadata$`Lifestyle`))
lfst$Type = "Lifestyle"
stats = rbind(state, age, lfst)
...
fill_colors = c(brewer.pal(5,"Blues"), brewer.pal(3,"Reds")[2:3], brewer.pal(3,"Greens"))
meta.plot = ggplot(stats, aes(x=Type, y=value, fill=Var1)) +
  geom_bar(stat="identity", alpha=0.8, colour="darkgrey") +
  theme_classic() +
  ylim(0,100) +
  scale_fill_manual(values = fill_colors) + 
  scale_x_discrete(limits=c("Health state", "Age group", "Lifestyle")) + 
  ylab("% Samples") + 
  theme(axis.title.y = element_text(size=14)) + 
  theme(axis.text.y = element_text(size=12)) + 
  theme(axis.title.x = element_blank()) + 
  theme(axis.text.x = element_text(size=12))
ggsave(file="../../../Publications/2022-Latinbiota/Figures/figure1/metadata_all_barplot.pdf", height=6, width=5)
